---
title: Reefer Container Shipment solution
...
In [this first chapter](./introduction/) we are presenting the business process for shipping fresh good over sees and we are detailing the analysis we have done in San Francisco in November 2108 using the event storming analysis workshop. You could read more on how to organize and execute such workshop [here](https://ibm-cloud-architecture.github.io/refarch-eda/methodology/event-storming/). The workshop execution transcript is detailed in [a second chapter](./analysis/readme).

## Design considerations

In the [third chapter](design/readme) we are detailing how to transform the analysis outcomes into some light design, conceptual view, just enough to start coding some microservices.

## Architecture

This quick [architecture chapter](design/architecture) presents the solution components working together with the event backbone.
...
* [Event Streams instance on IBM Cloud Public](https://ibm-cloud-architecture.github.io/refarch-eda/technology/event-streams/es-cloud/) or Event streams within IBM Cloud Pak for integration or a Kafka Strimzi Docker image.
...
Each project, part of the solution, has its own installation explanations and scripts to build, package, test and deploy to the different Kubernetes deployment (private and public). We recommend studying those scripts.
...
* **[Backing Services](deployments/backing-services)** documents the required environment configuration, as well as Kafka and Postgresql options to satisfy application dependencies.
* **[Application Components](deployments/application-components)** documents the deployment steps required for the K Container Reference Implementation, as well as integration tests to validate deployment.
