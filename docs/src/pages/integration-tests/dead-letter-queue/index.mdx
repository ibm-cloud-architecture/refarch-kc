...
This test case will test the scenario where the BPM service, which our reference application depends on, is not available. In such scenario, our application should be able to execute the [request retry or dead letter pattern](../../implementation/dead-letter-queue/) so that calls to the BPM service are retried, until certain threshold.
