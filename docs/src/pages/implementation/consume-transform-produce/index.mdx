...
Thanks to the Event Storming work we specifically did for our Reefer Container Shipment business domain (see it [here](../event-storming-analysis/), we were able to identify things like the **events and commands** that will flow through our system as well as the domains, subdomains, contexts, etc that will help us identify what microservices/components we should build.
