...
from kafka.KcAvroProducerES import KafkaProducer
from utils.avroEDAUtils import getContainerEventSchema, getContainerKeySchemaES

print(" @@@ Executing script: ProduceAvroContainerES.py")
...
if __name__ == '__main__':
    ...
    container_event_key_schema = getContainerKeySchemaES(DATA_SCHEMAS)
    # Create the container event and key to send
    container_event = createContainer(ID)
    container_event_key = {"containerID" : ID}
    ...
    kp.prepareProducer("ProduceAvroContainerESPython",container_event_key_schema,container_event_value_schema)
    kp.publishEvent(TOPIC_NAME,container_event,json.dumps(container_event_key))
