#!/bin/bash
kafka=$(docker ps -a | grep docker_kafka | awk '{print $NF}')
if [ -z "$kafka" ]
then
  echo "Kafka docker is not running"
fi
echo $kafka
# get current topics
docker exec  -ti $kafka /bin/bash -c "/opt/kafka/bin/kafka-topics.sh --list --zookeeper zookeeper1:2181" > topics
createTopic(){
    bc=$(tail -n+2 topics| grep  $1)
    echo $bc
    if [ -z "$bc" ]
    then
        echo "create " $1
        docker exec -ti docker_kafka1_1  /bin/bash -c "/opt/kafka/bin/kafka-topics.sh --create  --zookeeper zookeeper1:2181 --replication-factor 1 --partitions 1 --topic $1"
    else
        echo $1 " topic already created"
    fi
}

createTopic "bluewaterContainer" 
createTopic "bluewaterShip" 
createTopic "bluewaterProblem" 
createTopic "orders" 
createTopic "errors" 
createTopic "containers" 
createTopic "containerMetrics" 
createTopic "rejected-orders"
createTopic "allocated-orders"
rm topics
